// Projects Index - English Version
...
const ICONS = {
  ...
  bubble: 'M256 41a215 215 0 1 1 0 430a215 215 0 1 1 0-430zM182.5 205a95 95 0 1 0 0 190a95 95 0 1 0 0-190zM317.5 99.5a74 74 0 1 0 0 148a74 74 0 1 0 0-148zM363 269.5a57.5 57.5 0 1 0 0 115a57.5 57.5 0 1 0 0-115z',
};
const ICON_VIEWBOX = { code: '0 0 640 512', gamepad: '0 0 640 512', cube: '0 0 512 512', bubble: '0 0 512 512' };
...
const useInView = (options) => {
  ...
  useEffect(() => {
    ...
    const observer = new IntersectionObserver(([entry]) => { setInView(entry.isIntersecting); }, { threshold: options.threshold || 0 });
...
const ScrambledText = ({ text, phase }) => {
  ...
  useEffect(() => {
    ...
    let iteration = 0; const maxIterations = 24;
    const interval = setInterval(() => {
      setDisplayText(text.split('').map((_c, i) => i < Math.floor(iteration / (maxIterations / text.length)) ? text[i] : chars[Math.floor(Math.random() * chars.length)]).join(''));
      iteration++; if (iteration >= maxIterations) { clearInterval(interval); setDisplayText(text); }
    }, 50);
...
const MonumentalLink = ({ title, subtitle, link, index, color = "white", align = "left", iconName, scrollY, badge, subLinks }) => {
  ...
  useEffect(() => {
    ...
    const id = requestAnimationFrame(() => {
      ...
      setLogoStyle({ opacity: Math.pow(Math.max(0, 1 - dist / (window.innerHeight / 1.2)), 2) * 0.2 });
    });
  ...
  const iconEl = iconName && ICONS[iconName] && h('svg', { xmlns: 'http://www.w3.org/2000/svg', viewBox: ICON_VIEWBOX[iconName] || '0 0 512 512', fill: 'currentColor', className: 'absolute top-1/2 text-white z-[-2] pointer-events-none', style: { height: 'min(40vw, 400px)', opacity: logoStyle.opacity, [align === 'right' ? 'right' : 'left']: align === 'center' ? '50%' : '5%', transform: 'translate(' + (align === 'center' ? '-50%' : '0') + ', -50%)', willChange: 'opacity' } }, h('path', { d: ICONS[iconName], fillRule: 'evenodd' }));
  const titleStyle = { WebkitTextStroke: inView ? '0px transparent' : '2px #222', transform: inView ? 'translate3d(0,0,0)' : 'translate3d(' + (dir * 250) + 'px,' + (align === 'center' ? 100 : 0) + 'px,0)', transitionProperty: 'color, -webkit-text-stroke, transform', willChange: 'transform' };
  ...
  const titleEl = link ? h('a', { href: link, className: titleClass + ' cursor-pointer hover:opacity-70 ' + (inView ? activeColor : 'text-transparent'), style: titleStyle }, title) : h('span', { className: titleClass + ' cursor-default ' + (inView ? activeColor : 'text-transparent'), style: titleStyle }, title);
  const subtitleEl = h('div', { className: 'w-full max-w-3xl pt-2 flex flex-col relative z-20 ' + innerAlign },
    h('div', { className: 'transition-all duration-1000 ease-out delay-100 ' + (inView ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-12') },
      ...
      badge && h('span', { className: 'inline-block text-[10px] tracking-[0.15em] px-2 py-0.5 border border-[#00ff66]/40 text-[#00ff66]/70 font-mono mt-2 mb-1' }, badge),
      h('p', { className: 'text-base md:text-xl text-gray-400 font-light leading-relaxed' }, subtitle.desc),
      subLinks && h('div', { className: 'flex flex-wrap items-center gap-6 mt-6' },
        subLinks.map((sl, idx) => h('a', { key: idx, href: sl.url, target: '_blank', rel: 'noreferrer', className: 'text-xs md:text-sm font-mono tracking-widest transition-colors border-b border-transparent pb-1 uppercase text-white hover:border-[#00ff66] hover:text-[#00ff66]' }, '[ ', sl.name, ' ]'))
      )
    )
  );
  return h('div', { ref: setRefs, className: 'flex flex-col ' + flexAlign + ' gap-4 md:gap-6 py-12 px-4 relative z-10 w-full' }, iconEl, titleEl, subtitleEl);
};

const globalCSS = `
@font-face { font-family: 'Smooch Sans'; src: url('../../HomePageAssets/SmoochSans-VariableFont_wght.ttf') format('truetype'); font-weight: 100 900; font-style: normal; font-display: swap; }
body { -webkit-font-smoothing: antialiased; -moz-osx-font-smoothing: grayscale; text-rendering: optimizeLegibility; font-family: 'Smooch Sans', system-ui, sans-serif; }
.font-display { font-family: 'Anton', sans-serif; }
::-webkit-scrollbar { width: 8px; } ::-webkit-scrollbar-track { background: #050505; } ::-webkit-scrollbar-thumb { background: #333; } ::-webkit-scrollbar-thumb:hover { background: #00ff66; }
...
const ProjectsPage = () => {
  ...
  return h('div', { className: 'min-h-screen bg-[#050505] text-[#f0f0f0] overflow-hidden selection:bg-[#00ff66] selection:text-black', style: { fontFamily: "'Smooch Sans', system-ui, sans-serif" } },
    ...
    h('main', { className: 'relative z-10 w-full' },
      h('header', { className: 'relative w-full h-screen flex flex-col items-center justify-center' },
        h('div', { className: 'text-center' },
          h('h1', { 'data-text': 'PROJECTS', className: 'text-[5rem] sm:text-[8rem] md:text-[10rem] lg:text-[12rem] tracking-tighter uppercase leading-[0.8] text-white font-display transform-gpu relative ' + (introPhase !== 'done' ? 'animate-brutal-glitch' : ''), style: { willChange: 'transform' } }, h(ScrambledText, { text: 'PROJECTS', phase: introPhase })),
          h('p', { className: 'text-[#00ff66] tracking-[0.5em] text-xs md:text-sm font-mono mt-6 opacity-80' }, '// MATURE PROJECTS'),
          h('p', { className: 'text-gray-500 text-sm mt-4 max-w-md mx-auto leading-relaxed' }, 'Completed project entries.')
        ),
        h('div', { className: 'absolute bottom-4 md:bottom-8 left-1/2 -translate-x-1/2 flex flex-col items-center gap-1 transition-opacity duration-500 ' + (scrollY > 50 ? 'opacity-0 pointer-events-none' : 'opacity-70') },
          ...
          h('svg', { className: 'w-4 h-4 md:w-5 md:h-5 text-[#00ff66] animate-bounce-down', viewBox: '0 0 24 24', fill: 'none', stroke: 'currentColor', strokeWidth: '2' }, h('path', { d: 'M7 13l5 5 5-5M7 6l5 5 5-5' }))
        )
      ),
      h('section', { className: 'relative w-full py-24 md:py-36 z-20 bg-[#020202]' },
        h('div', { className: 'max-w-[100rem] mx-auto px-6 w-full' },
          ...
          h('div', { className: 'flex flex-col gap-16 md:gap-32 w-full' },
            ...
            h(MonumentalLink, { scrollY, title: 'POCHAN', subtitle: { label: 'github.com/Lavr0v0/Pochan', desc: 'Pochan — A local-first anime tracker with bubble visualization' }, link: 'https://github.com/Lavr0v0/Pochan', index: 2, color: 'green', align: 'left', iconName: 'bubble', badge: 'DESKTOP APP', subLinks: [{ name: 'GitHub', url: 'https://github.com/Lavr0v0/Pochan' }, { name: 'Releases', url: 'https://github.com/Lavr0v0/Pochan/releases' }] }),
            h(MonumentalLink, { scrollY, title: 'CARPET LIR', subtitle: { label: 'modrinth.com/mod/carpet-lir-addition', desc: "Lavro's Item Renewability Carpet Addition — A Minecraft Carpet extension mod" }, link: 'https://modrinth.com/mod/carpet-lir-addition', index: 3, color: 'white', align: 'right', iconName: 'cube', badge: 'MINECRAFT MOD', subLinks: [{ name: 'Modrinth', url: 'https://modrinth.com/mod/carpet-lir-addition' }, { name: 'GitHub', url: 'https://github.com/Lavr0v0/carpet-lir-addition' }] })
          )
        )
      ),
      h('section', { className: 'relative w-full bg-black z-20 pb-24 pt-12 border-t border-[#111]' },
        h('footer', { className: 'pt-8 flex flex-col items-center gap-8' },
          h('button', { onClick: () => { if (lenisRef.current) lenisRef.current.scrollTo(0, { duration: 2 }); else window.scrollTo({ top: 0, behavior: 'smooth' }); }, className: 'group flex flex-col items-center gap-2 cursor-pointer transition-all duration-300 hover:text-[#00ff66] text-gray-600' },
            h('svg', { className: 'w-4 h-4 transition-transform duration-300 group-hover:-translate-y-1', viewBox: '0 0 24 24', fill: 'none', stroke: 'currentColor', strokeWidth: '2' }, h('path', { d: 'M17 11l-5-5-5 5M17 18l-5-5-5 5' })),
          ...
          h('a', { href: '/en/', className: 'font-mono text-xs tracking-[0.2em] text-gray-500 hover:text-[#00ff66] transition-colors duration-300 border border-[#1a1a1a] hover:border-[#00ff66] px-6 py-2' }, '[ BACK TO HOME ]'),
          h('p', { className: 'font-mono text-xs text-gray-600 mt-4' }, 'LAVRO.ORG \u00A9 ' + new Date().getFullYear() + ' // PROJECTS')
        )
      )
    )
  );
...
requestAnimationFrame(function() { requestAnimationFrame(function() { document.getElementById('root').style.transition = 'opacity 0.3s ease'; document.getElementById('root').style.opacity = '1'; }); });
